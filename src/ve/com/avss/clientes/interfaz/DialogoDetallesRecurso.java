...
public class DialogoDetallesRecurso extends AbstractDialogoBaseRecurso {

	private static final long serialVersionUID = 1L;
	private static final Logger log = Logger.getLogger(DialogoDetallesRecurso.class);
	private List<DatosTarea> listaTareas = new ArrayList<DatosTarea>();
	private DatosClientesProveedor datosRecurso = new DatosClientesProveedor();
	

	private int rowSeleccionada = 0;
	private String actionCommand = "";
	
	public DialogoDetallesRecurso(JFrame parent, DatosClientesProveedor datosRecurso,DatosUsuario datosUsuario) throws Exception { 
		super(parent);
		log.info("Constructor - construyendo paneles e interfaz");
		
		//lista de tarear por proyecto
		//datos del recurso
		...
		eDescripcion.setText(eDescripcion.getText()+" "+datosRecurso.getNombre()+" "+datosRecurso.getApellido());
		eTelefono.setText(eTelefono.getText()+" "+datosRecurso.getTelefono());
		eCorreo.setText(eCorreo.getText()+" "+datosRecurso.getCorreo());
		...
		btnActualizarTarea.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				//asignado
				String asignado = datosRecurso.getIdBD();
				//observacion
				...
				try {
					...
					tarea.setProyecto(listaProyectosId.get(comboProyectosTareaAct.getSelectedItem()));
					...
					comboProyectosTareaAct.setSelectedIndex(0);
		...
		btnCrearTarea.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				panelRecurso.add(eProyectoTarea);
				panelRecurso.add(comboProyectoTarea);
				...
				tarea.setRecurso(datosRecurso.getIdBD());
				tarea.setProyecto(listaProyectosId.get(comboProyectoTarea.getSelectedItem()));
				...
				if (result == 1) {
					...
					comboProyectoTarea.setSelectedIndex(0);
		...
		btnReportePDF.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				List<DatosTarea> listaTareasReporte = QueryReporte.obtenerListaTareasPorRecurso(datosRecurso.getIdBD(), condicionOperador, sDate+"")	;
				//datos de reporte
				DatosReporte datosReporte = new DatosReporte();
				
				//datos de recursos
				datosReporte.setDatosRecurso(datosRecurso);
				//lista de tareas
		...
		panelDatos.add(comboProyectosTareaAct);
	...
	private void cargarListaTareas() {
		...
		limpiartabla();
		try {
			listaTareas = QueryProyecto.obtenerListaTareaPorRecurso(datosRecurso.getIdBD());
		} catch (Exception e) {
		...
		tablaListaTareas.addMouseListener(new MouseAdapter() {
	         public void mouseClicked(MouseEvent me) {
	            if (me.getClickCount() == 1 ||me.getClickCount() == 2) {     // to detect doble click events
	               ...
	               String descripcion = tablaListaTareas.getValueAt(row, 4).toString();
	               textAreaDescripcion.setText(descripcion);
	               
	               String status = tablaListaTareas.getValueAt(row, 1).toString();
	               
	               String proyecto = tablaListaTareas.getValueAt(row, 2).toString();
	               
	               String fecha = tablaListaTareas.getValueAt(row, 3).toString();
	               ...
	               comboProyectosTareaAct.setSelectedItem(proyecto);
	...
	private void construirTablaTareas() {
		...
		// Column Names
		String[] nombreColumnaProductos = {"id", "Status","Proyecto","Fecha", "Descripción", "Observación" };
	    ...
	    //fecha
	    column = tablaListaTareas.getColumnModel().getColumn(3);
	    column.setMinWidth(80);
	    column.setMaxWidth(100);
	    column.setWidth(80);
	    column.setPreferredWidth(100);
